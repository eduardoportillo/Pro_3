...
public class Imagen {
    ...
    public Imagen(String URL) {
        ...
        try {
            ...
            System.out.println("Error al cambiar a buffered");
        }
    }
    ...
    public void iniciarPintarParecidos(int x, int y) {
        new Thread() {
            public void run() {
                ...
                System.out.println("CANTIDAD DE SELECCIONADOS = " + cantidadBlancos + " / " + totalPixel);
            ...
            try {
                if ((x - 1) > 0 && !dir.equals("abajo")) {
                    if (pixels[y][(x - 1)].rgbaToInt() == base.rgbaToInt()
                            /* new code */ && pixels[y][(x - 1)].getVerificado() != true) {
                        pintarParecidos((x - 1), y, base, pixels, "arriba");
                        /* new code */ pixels[y][(x - 1)].setVerificado(true);
                    }
                }
            } catch (Exception e) {
            ...
            try {
                if ((y + 1) <= pixels.length && !dir.equals("arriba")) {
                    if (pixels[(y + 1)][x].rgbaToInt() == base.rgbaToInt()
                            /* new code */ && pixels[(y + 1)][x].getVerificado() != true) {
                        pintarParecidos(x, (y + 1), base, pixels, "abajo");

                        /* new code */ pixels[(y + 1)][x].setVerificado(true);
                    }
                }
            } catch (Exception e) {
